...
exports["default"] = exports.ModelFactory = void 0;
...
var ModelFactory = {
  ...
  createGeppettoModel: function createGeppettoModel(jsonModel, storeModel, populateRefs) {
    ...
    if (jsonModel.eClass == 'GeppettoModel') {
      ...
      if (storeModel) {
        // store raw model for easy access during model building operations
        this.rawGeppetoModel = jsonModel; // store object model
      ...
      geppettoModel.datasources = this.createDatasources(jsonModel.dataSources, geppettoModel); // create top level queries (potentially cross-datasource)
      ...
      if (populateRefs) {
        ...
        this.populateChildrenShortcuts(geppettoModel); // traverse everything and populate type references in variables
  ...
  createStaticInstance: function createStaticInstance(rawInstance) {
    ...
    switch (rawInstance.eClass) {
      case _Resources["default"].SIMPLE_INSTANCE_NODE:
        instance = new SimpleInstance(rawInstance);
        break;

      case _Resources["default"].SIMPLE_CONNECTION_INSTANCE_NODE:
    ...
    if (typeof node.getChildren === "function") {
      ...
      if (children != undefined) {
        for (var i = 0; i < children.length; i++) {
          ...
          if (node.getMetaType() != _Resources["default"].ARRAY_INSTANCE_NODE) {
  ...
  populateTypeReferences: function populateTypeReferences(node) {
    ...
    if (node.getMetaType() == _Resources["default"].VARIABLE_NODE) {
      ...
      if (types != undefined) {
        for (var i = 0; i < types.length; i++) {
          ...
          var refStr = types[i].$ref; // if it's anonymous there's no reference

          if (refStr != undefined) {
            // go grab correct type from Geppetto Model
            var typeObj = this.resolve(refStr); // track if we have pointer type

            if (typeObj.getMetaType() == _Resources["default"].POINTER_TYPE) {
              hasPointerType = true;
            } // add to list
      ...
      if (hasPointerType) {
        ...
        if (initialValues != undefined && initialValues.length == 1) {
          ...
          var pointer = this.createPointer(val.value); // populate pointerValue on variable
      ...
      for (var j = 0; j < resolvedTypes.length; j++) {
        if (resolvedTypes[j].getMetaType() == _Resources["default"].PARAMETER_TYPE) {
          // if a variable has a Parameter type, add AParameterCapability to the variable
          node.extendApi(AParameterCapability);
        } else if (resolvedTypes[j].getMetaType() == _Resources["default"].CONNECTION_TYPE) {
      ...
      if (superType != undefined) {
        var typeObjs = []; // convert to array if single element
    ...
    } else if (node.getMetaType() === _Resources["default"].SIMPLE_INSTANCE_NODE || node.getMetaType() === _Resources["default"].SIMPLE_CONNECTION_INSTANCE_NODE) {
  ...
  createPointer: function createPointer(jsonPointer) {
    ...
    var pointerElements = []; // loop elements and create PointerElements (resolving variables / types)
  ...
  createPointerElement: function createPointerElement(jsonPointerElement) {
    ...
    var index = jsonPointerElement.index; // create pointer object setting elements
    ...
    if (jsonVariables != undefined) {
      for (var i = 0; i < jsonVariables.length; i++) {
        if (!jsonVariables[i].synched) {
          ...
          variable.parent = parent; // check if it has an anonymous type
    ...
    if (jsonTypes != undefined) {
      for (var i = 0; i < jsonTypes.length; i++) {
        if (!jsonTypes[i].synched) {
          var type = null; // check if it's composite type, visual type, array type or simple type
          ...
          if (jsonTypes[i].eClass == 'CompositeType' || jsonTypes[i].eClass == 'ConnectionType') {
            ...
            type = this.createCompositeVisualType(jsonTypes[i]); // inject visual capability to all CompositeVisualType

            type.extendApi(AVisualCapability);
          } else if (jsonTypes[i].eClass == 'ImportType') {
            type = this.createImportType(jsonTypes[i], null); // we store the index of the importType to speed up swapping procedures

            type._index = i;
          } else if (jsonTypes[i].eClass == 'ArrayType') {
            type = this.createArrayType(jsonTypes[i]);
          } else {
            type = this.createType(jsonTypes[i]); // inject visual capability if MetaType == VisualType

            if (type.getMetaType() == _Resources["default"].VISUAL_TYPE_NODE) {
              type.extendApi(AVisualCapability);
            }
  ...
  instantiateVariables: function instantiateVariables(geppettoModel) {
    var instances = []; // we need to explode instances for variables with visual types

    var varsWithVizTypes = []; // we need to fetch all potential instance paths (even for not exploded instances)

    var allPotentialInstancePaths = [];
    var allPotentialInstancePathsForIndexing = []; // builds list of vars with visual types and connection types - start traversing from top level variables
    ...
    var varsToInstantiate = varsWithVizTypes; // based on list, traverse again and build instance objects
    ...
    this.instances = instances; // populate shortcuts / populate connection references
  ...
  createInstancesFromDiffReport: function createInstancesFromDiffReport(diffReport) {
    ...
    var getPotentialInstancePaths = function getPotentialInstancePaths(types) {
      ...
      for (var l = 0; l < types.length; l++) {
        if (types[l].hasCapability(_Resources["default"].VISUAL_CAPABILITY)) {
    ...
    newInstancePaths = newInstancePaths.concat(getPotentialInstancePaths(diffTypes)); // STEP 3: call getInstance to create the instances

    var newInstances = window.Instances.getInstance(newInstancePaths); // STEP 4: If instances were added, re-populate shortcuts
  ...
  populateConnections: function populateConnections(instance) {
    ...
    if (instance.getMetaType() === _Resources["default"].SIMPLE_CONNECTION_INSTANCE_NODE) {
    ...
    {
      if (instance.getType().getMetaType() == _Resources["default"].CONNECTION_TYPE) {
  ...
  mergeModel: function mergeModel(rawModel, overrideTypes) {
    ...
    this.newPathsIndexing = []; // diff object to report back what changed / has been added
    ...
    var diffModel = this.createGeppettoModel(rawModel, false, false); // STEP 2: add libraries/types if any are different (both to object model and json model)
    ...
    for (var i = 0; i < diffLibs.length; i++) {
      ...
      for (var j = 0; j < libs.length; j++) {
        ...
        if (diffLibs[i].getPath() == libs[j].getPath()) {
          ...
          var existingTypes = libs[j].getTypes(); // first loop on types - add new ones
          ...
          for (var k = 0; k < diffTypes.length; k++) {
            ...
            if (!typeMatch) {
              ...
              libs[j].getWrappedObj().types.push(diffTypes[k].getWrappedObj()); // add to library in geppetto object model
              ...
              diffReport.types.push(diffTypes[k]); // populate the shortcuts for the added type

              this.populateChildrenShortcuts(diffTypes[k]); // let's populate the shortcut in the parent of the type, this might not exist if it was a fetch
          ...
          if (overrideTypes) {
            for (var k = 0; k < typeMatched.length; k++) {
              ...
              var variablesToUpdate = importTypeMatched[k].getVariableReferences(); // swap type reference in ALL variables that point to it
              ...
              libs[j].getWrappedObj().types[index] = typeMatched[k].getWrappedObj(); // store overridden type (so that unresolve type can swap it back)

              typeMatched[k].overrideType = importTypeMatched[k]; // swap in object model

              typeMatched[k].parent = libs[j];
              libs[j].getTypes()[index] = typeMatched[k]; // libs[j].removeImportType(importTypeMatched[k]);
              // add potential instance paths

              this.addPotentialInstancePathsForTypeSwap(typeMatched[k]); // update capabilities for variables and instances if any

              this.updateCapabilities(variablesToUpdate); // add to diff report

              diffReport.types.push(typeMatched[k]); // populate the shortcuts for the swapped type

              this.populateChildrenShortcuts(typeMatched[k]); // let's populate the shortcut in the parent of the type, this might not exist if it was a fetch
      ...
      if (!libMatch) {
        ...
        this.geppettoModel.getWrappedObj().libraries.push(diffLibs[i].getWrappedObj()); // add to geppetto object model

        diffLibs[i].parent = this.geppettoModel;
        this.geppettoModel.getLibraries().push(diffLibs[i]); // add to diff report

        diffReport.libraries.push(diffLibs[i]); // populate the shortcuts for the added library

        this.populateChildrenShortcuts(diffLibs[i]); // let's populate the shortcut in the parent of the library, this might not exist if it was a fetch
    ...
    var currentWorld = this.geppettoModel.getCurrentWorld(); // STEP 3b: merge world.variables and instances
    ...
    if (currentWorld) {
      ...
      diffReport.worlds[0].variables = diffReport.worlds[0].variables.concat(this._mergeVariables(diffVars, currentWorld)); // TODO handle multiple worlds
  ...
  _mergeVariables: function _mergeVariables(diffVars, parent) {
    ...
    for (var x = 0; x < diffVars.length; x++) {
      ...
      if (!match) {
        ...
        wrappedObj.variables.push(diffVars[x].getWrappedObj()); // add variable to geppetto object model

        diffVars[x].parent = parent;
        currentModelVars.push(diffVars[x]); // populate references for new vars

        this.populateTypeReferences(diffVars[x]); // find new potential instance paths and add to the list

        this.addPotentialInstancePaths([diffVars[x]]);
        diffReportVars.push(diffVars[x]); // populate the shortcuts for the added variable

        this.populateChildrenShortcuts(diffVars[x]); // let's populate the shortcut in the parent of the variable, this might not exist if it was a fetch
  ...
  _mergeInstances: function _mergeInstances(diffInst, parent) {
    ...
    for (var x = 0; x < diffInst.length; x++) {
      ...
      if (match) {
        ...
        // if no match, add it, it's actually new
        diffReportInst.push(diffInst[x]); // append variable to raw model

        wrappedObj.instances.push(diffInst[x].getWrappedObj()); // add variable to geppetto object model

        currentModelInst.push(diffInst[x]); // find new potential instance paths and add to the list

        var newInstancePath = createInstancePathObj(diffInst[x]);
        this.allPaths.push(newInstancePath);
        this.allPathsIndexing.push(newInstancePath); // let's populate the shortcut in the parent of the variable, this might not exist if it was a fetch
  ...
  mergeValue: function mergeValue(rawModel, overrideTypes) {
    ...
    var diffVars = diffReport.variables; // STEP 1: create new geppetto model to merge into existing one

    var diffModel = this.createGeppettoModel(rawModel, false, false); // STEP 1.5: add world
  ...
  updateCapabilities: function updateCapabilities(variables) {
    ...
    var updateInstancesCapabilities = function updateInstancesCapabilities(instances) {
      for (var j = 0; j < instances.length; j++) {
        // check if visual type and inject AVisualCapability
        var visualType = instances[j].getVisualType(); // check if visual type and inject AVisualCapability

        if (!(visualType instanceof Array) && visualType != null && visualType != undefined || visualType instanceof Array && visualType.length > 0) {
          if (!instances[j].hasCapability(_Resources["default"].VISUAL_CAPABILITY)) {
        ...
        if (instances[j].getType().getMetaType() == _Resources["default"].CONNECTION_TYPE) {
          if (!instances[j].hasCapability(_Resources["default"].CONNECTION_CAPABILITY)) {
            instances[j].extendApi(AConnectionCapability);
            that.resolveConnectionValues(instances[j]);
          }
        }

        if (instances[j].getType().getMetaType() == _Resources["default"].STATE_VARIABLE_TYPE) {
          if (!instances[j].hasCapability(_Resources["default"].STATE_VARIABLE_CAPABILITY)) {
            instances[j].extendApi(AStateVariableCapability);
          }
        }

        if (instances[j].getType().getMetaType() == _Resources["default"].DERIVED_STATE_VARIABLE_TYPE) {
          if (!instances[j].hasCapability(_Resources["default"].DERIVED_STATE_VARIABLE_CAPABILITY)) {
            instances[j].extendApi(ADerivedStateVariableCapability);
          }
        }

        if (instances[j].getType().getMetaType() == _Resources["default"].PARAMETER_TYPE) {
          if (!instances[j].hasCapability(_Resources["default"].PARAMETER_CAPABILITY)) {
    ...
    for (var i = 0; i < variables.length; i++) {
      ...
      for (var j = 0; j < resolvedTypes.length; j++) {
        if (resolvedTypes[j].getMetaType() == _Resources["default"].PARAMETER_TYPE) {
          // if a variable has a Parameter type, add AParameterCapability to the variable
          if (!variables[i].hasCapability(_Resources["default"].PARAMETER_CAPABILITY)) {
            variables[i].extendApi(AParameterCapability);
          }
        } else if (resolvedTypes[j].getMetaType() == _Resources["default"].CONNECTION_TYPE) {
          // if a variable has a connection type, add connection capability
          if (!variables[i].hasCapability(_Resources["default"].CONNECTION_CAPABILITY)) {
            variables[i].extendApi(AConnectionCapability);
          }
        }
      }

      var varInstances = this.getAllInstancesOf(variables[i]); // update instances capabilities
  ...
  addPotentialInstancePathsForTypeSwap: function addPotentialInstancePathsForTypeSwap(type) {
    var typePath = type.getPath(); // Get all paths for the new type

    var partialPathsForNewType = [];
    var partialPathsForNewTypeIndexing = [];
    this.fetchAllPotentialInstancePathsForType(type, partialPathsForNewType, partialPathsForNewTypeIndexing, []); // Get all potential instances for the type we are swapping
    ...
    this.allPaths.replace = []; // Generate new paths and add
    ...
    this.newPathsIndexing.replace = []; // same as above for indexing paths
  ...
  swapTypeInTypes: function swapTypeInTypes(types, typeToSwapOut, typeToSwapIn) {
    for (var y = 0; y < types.length; y++) {
      ...
      } else if (types[y].getMetaType() == _Resources["default"].COMPOSITE_TYPE_NODE) {
   ...
   */
  addInstances: function addInstances(newInstancesPaths, topInstances, geppettoModel) {
    var instanceCreatedCallback = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;
    ...
    for (var j = 0; j < newInstancesPaths.length; j++) {
      ...
      for (var i = 0; i < splitInstancePath.length; i++) {
        if (splitInstancePath[i].indexOf('[') > -1) {
          ...
          var arrayId = splitInstancePath[i].split('[')[0]; // replace brackets
    ...
    for (var k = 0; k < topInstances.length; k++) {
      this.populateChildrenShortcuts(topInstances[k]); // populate at window level
    ...
    if (instanceCreatedCallback) {
      newInstancesPaths.forEach(function (newInstance) {
        if (newInstance !== "time") {
          if (newInstance.includes('.')) {
            var instanceStrings = newInstance.split(".");
   ...
   */
  buildInstanceHierarchy: function buildInstanceHierarchy(path, parentInstance, model, topLevelInstances) {
    var variable = null;
    var newlyCreatedInstance = null;
    var newlyCreatedInstances = []; // STEP 1: find matching first variable in path in the model object passed in

    var varsIds = path.split('.'); // check model MetaType and find variable accordingly

    if (model.getMetaType() == _Resources["default"].GEPPETTO_MODEL_NODE) {
    ...
    } else if (model.getMetaType() == _Resources["default"].VARIABLE_NODE) {
      var allTypes = model.getTypes(); // if array, and the array type

      if (allTypes.length == 1 && allTypes[0].getMetaType() == _Resources["default"].ARRAY_TYPE_NODE) {
        allTypes.push(model.getTypes()[0].getType());
      } // get all variables and match it from there


      for (var i = 0; i < allTypes.length; i++) {
        if (allTypes[i].getMetaType() == _Resources["default"].COMPOSITE_TYPE_NODE) {
      ...
      if (variable == null && parentInstance.getMetaType() == _Resources["default"].ARRAY_INSTANCE_NODE) {
    ...
    if (variable != null) {
      ...
      for (var j = 0; j < types.length; j++) {
        if (types[j].getMetaType() == _Resources["default"].ARRAY_TYPE_NODE) {
          arrayType = types[j];
          break;
        }
      ...
      if (matchingInstance != null) {
        ...
        var size = arrayType.getSize(); // create new ArrayInstance object, add children to it

        var arrayOptions = {
          id: variable.getId(),
          name: variable.getName(),
          _metaType: _Resources["default"].ARRAY_INSTANCE_NODE,
          variable: variable,
          size: size,
        ...
        for (var i = 0; i < size; i++) {
          ...
          var options = {
            ...
            _metaType: _Resources["default"].ARRAY_ELEMENT_INSTANCE_NODE,
            variable: variable,
            children: [],
            parent: arrayInstance,
            index: i
          };
          var explodedInstance = this.createArrayElementInstance(options); // check if visual type and inject AVisualCapability
          ...
          if (explodedInstance.getType().getMetaType() == _Resources["default"].CONNECTION_TYPE) {
            explodedInstance.extendApi(AConnectionCapability);
            this.resolveConnectionValues(explodedInstance);
          }

          if (explodedInstance.getType().getMetaType() == _Resources["default"].STATE_VARIABLE_TYPE) {
            explodedInstance.extendApi(AStateVariableCapability);
          }

          if (explodedInstance.getType().getMetaType() == _Resources["default"].DERIVED_STATE_VARIABLE_TYPE) {
            explodedInstance.extendApi(ADerivedStateVariableCapability);
          }

          if (explodedInstance.getType().getMetaType() == _Resources["default"].PARAMETER_TYPE) {
          ...
          arrayInstance[i] = explodedInstance; // ad to newly created instances list
        ...
        var options = {
          id: variable.getId(),
          name: variable.getName(),
          _metaType: _Resources["default"].INSTANCE_NODE,
          variable: variable,
          children: [],
          parent: parentInstance
        };
        newlyCreatedInstance = this.createInstance(options); // check if visual type and inject AVisualCapability

        var visualType = newlyCreatedInstance.getVisualType(); // check if visual type and inject AVisualCapability

        if (!(visualType instanceof Array) && visualType != null && visualType != undefined || visualType instanceof Array && visualType.length > 0) {
          newlyCreatedInstance.extendApi(AVisualCapability); // particles can move, we store its state in the time series coming from the statevariablecapability

          if (visualType.getId() == _Resources["default"].PARTICLES_TYPE) {
        ...
        if (newlyCreatedInstance.getType().getMetaType() == _Resources["default"].CONNECTION_TYPE) {
          newlyCreatedInstance.extendApi(AConnectionCapability);
          this.resolveConnectionValues(newlyCreatedInstance);
        }

        if (newlyCreatedInstance.getType().getMetaType() == _Resources["default"].STATE_VARIABLE_TYPE) {
          newlyCreatedInstance.extendApi(AStateVariableCapability);
        }

        if (newlyCreatedInstance.getType().getMetaType() == _Resources["default"].DERIVED_STATE_VARIABLE_TYPE) {
          newlyCreatedInstance.extendApi(ADerivedStateVariableCapability);
        }

        if (newlyCreatedInstance.getType().getMetaType() == _Resources["default"].PARAMETER_TYPE) {
   ...
   */
  resolveConnectionValues: function resolveConnectionValues(connectionInstanceOrVariable) {
    // get initial values
    ...
    } else if (connectionInstanceOrVariable.getMetaType() == _Resources["default"].VARIABLE_NODE) {
      initialValues = connectionInstanceOrVariable.getWrappedObj().initialValues;
    } // get pointer A and pointer B


    var connectionValue = initialValues[0].value; // resolve A and B to Pointer Objects
   ...
   */
  augmentPointer: function augmentPointer(pointer, connectionInstance) {
    // find root for this branch
    var rootInstance = this.findRoot(connectionInstance); // find instance for given pointed variable if any

    var pointedVariable = pointer.getElements()[0].getVariable();
    var pointedIndex = pointer.getElements()[0].getIndex(); // TODO: this could return potentially more than one match - need to extend to resolve to one

    var matchingInstance = this.findMatchingInstanceByID(pointedVariable.getId(), [rootInstance]); // traverse branch and build new array of PointerElements down to instance, given instancepath

    var pointerElements = [];
    var originalElement = pointer.getElements()[0];
    this.buildPointerElementsChain(matchingInstance.getRawInstancePath(), rootInstance, pointerElements, originalElement); // horribly override elements with newly created ones

    pointer.elements = pointerElements; // add connection instance reference to matching instance for easy retrieval
   ...
   *
   */
  buildPointerElementsChain: function buildPointerElementsChain(path, instance, pointerElements, originalElement) {
   ...
   * Find root instance
   */
  findRoot: function findRoot(instance) {
    var matching = null;
   ...
   */
  propagateCapabilityToParents: function propagateCapabilityToParents(capability, instance) {
    var parent = instance.getParent(); // check if it has capability
   ...
   */
  findMatchingInstanceByID: function findMatchingInstanceByID(id, instances) {
    var matching = null;
   ...
   */
  findMatchingInstance: function findMatchingInstance(instancePath, instances) {
    var matching = null;
   ...
   */
  findMatchingInstancesByType: function findMatchingInstancesByType(type, instances, matchingInstance) {
    for (var i = 0; i < instances.length; i++) {
   ...
   */
  findMatchingInstancesByVariable: function findMatchingInstancesByVariable(variable, instances, matchingInstance) {
    for (var i = 0; i < instances.length; i++) {
   ...
   */
  fetchVarsWithVisualTypes: function fetchVarsWithVisualTypes(node, varsWithVizTypes, parentPath) {
    /*
    ...
    if (node.getMetaType() == _Resources["default"].VARIABLE_NODE) {
      ...
      for (var i = 0; i < allTypes.length; i++) {
        ...
        if (allTypes[i].getMetaType() == _Resources["default"].TYPE_NODE || allTypes[i].getMetaType() == _Resources["default"].COMPOSITE_TYPE_NODE) {
        ...
        } else if (allTypes[i].getMetaType() == _Resources["default"].ARRAY_TYPE_NODE) {
        ...
        } else if (allTypes[i].getMetaType() == _Resources["default"].VISUAL_TYPE_NODE || allTypes[i].getMetaType() == _Resources["default"].COMPOSITE_VISUAL_TYPE_NODE) {
          varsWithVizTypes.push(path);
        ...
        if (allTypes[i].getMetaType() == _Resources["default"].COMPOSITE_TYPE_NODE) {
          ...
          var arrayType = allTypes[i].getType(); // check if the array is of composite type and if so recurse too on contained variables

          if (arrayType.getMetaType() == _Resources["default"].COMPOSITE_TYPE_NODE) {
   ...
   * @returns {boolean}
   */
  includePotentialInstance: function includePotentialInstance(node, path) {
    if (node.getType().getMetaType() == _Resources["default"].CONNECTION_TYPE) {
      return false;
    }

    if (node.getType().getMetaType() == _Resources["default"].TEXT_TYPE) {
      return false;
    }

    var nested = this.getNestingLevel(path);

    if (node.getType().getMetaType() == _Resources["default"].COMPOSITE_TYPE_NODE && nested > 2) {
      return false;
    }
   ...
   * @returns {number}
   */
  getNestingLevel: function getNestingLevel(path) {
   ...
   */
  printInstanceStats: function printInstanceStats() {
    var stats = {};
   ...
   */
  fetchAllPotentialInstancePaths: function fetchAllPotentialInstancePaths(node, allPotentialPaths, allPotentialPathsForIndexing, parentPath) {
    // build new path
    var xpath = '';
    var nodeRef = node;
    ...
    if (!isStaticVar || isStaticVar && !staticVarAlreadyAdded) {
      allPotentialPaths.push(entry); // only add to indexing if it's not a connection or nested in a composite type
    ...
    var potentialParentPaths = []; // check meta type - we are only interested in NON-static variables
    ...
    if (nodeRef instanceof Variable && !node.isStatic()) {
      ...
      for (var m = 0; m < allTypes.length; m++) {
        if (allTypes[m].getMetaType() == _Resources["default"].ARRAY_TYPE_NODE) {
      ...
      if (arrayType != undefined) {
        ...
        var arrayMetaType = arrayType.getType().getMetaType(); // add the [*] entry
   ...
   */
  fetchAllPotentialInstancePathsForType: function fetchAllPotentialInstancePathsForType(type, allPotentialPaths, allPotentialPathsForIndexing, potentialParentPaths) {
    if (type.getMetaType() == _Resources["default"].COMPOSITE_TYPE_NODE) {
    ...
    } else if (type.getMetaType() == _Resources["default"].ARRAY_TYPE_NODE) {
      var arrayType = type.getType(); // check if the array is of composite type and if so recurse too on contained variables
  ...
  createModel: function createModel(node, options) {
  ...
  /** Creates a simple composite */
  createLibrary: function createLibrary(node, options) {
    if (options == null || options == undefined) {
      options = {
        wrappedObj: node
      };
  ...
  /** Creates a variable */
  createVariable: function createVariable(node, options) {
    if (options == null || options == undefined) {
      options = {
        wrappedObj: node,
        types: node.types
  ...
  createValues: function createValues(initialValuesObject, variable) {
    var values = [];
    var options;
  ...
  createValue: function createValue(valueNode, options) {
    if (options == null || options == undefined) {
      options = {
  ...
  createDatasource: function createDatasource(node, options) {
    if (options == null || options == undefined) {
      options = {
        wrappedObj: node
      };
    ...
    var d = new Datasource(options); // create queries
   ...
   * @returns {Array}
   */
  createQueries: function createQueries(rawQueries, parent) {
    var queries = [];

    if (rawQueries != undefined) {
      for (var i = 0; i < rawQueries.length; i++) {
        var q = this.createQuery(rawQueries[i]); // set datasource as parent

        q.parent = parent; // push query to queries array
  ...
  createQuery: function createQuery(node, options) {
    if (options == null || options == undefined) {
      options = {
        wrappedObj: node
      };
    ...
    var q = new Query(options); // set matching criteria
    ...
    if (node.matchingCriteria != undefined) {
      for (var i = 0; i < matchingCriteriaRefs.length; i++) {
        ...
        for (var j = 0; j < typeRefs.length; j++) {
          ...
          var type = this.resolve(ref); // push to q.matchingCriteria
  ...
  getTypeOptions: function getTypeOptions(node, options) {
    if (options == null || options == undefined) {
      return {
        wrappedObj: node,
  ...
  /** Creates a type */
  createType: function createType(node, options) {
    var _this2 = this;

    var t = new Type(this.getTypeOptions(node, options));

    if (node.tags) {
      t.tags = node.tags.map(function (tag) {
        return _this2.resolve(tag.$ref);
      });
    }
  ...
  createImportType: function createImportType(node, options) {
    var it = new ImportType(this.getTypeOptions(node, options));
    return it;
  },

  /** Creates a composite type */
  createCompositeType: function createCompositeType(node, options) {
  ...
  createCompositeVisualType: function createCompositeVisualType(node, options) {
  ...
  /** Creates a composite type */
  createArrayType: function createArrayType(node, options) {
    var t = new ArrayType(this.getTypeOptions(node, options));
    t.size = node.size;
    t.type = node.arrayType;
    return t;
  },
  updateConnectionInstances: function updateConnectionInstances(instance) {
    var typesToSearch = this.getAllTypesOfMetaType(_Resources["default"].COMPOSITE_TYPE_NODE);
    var connectionVariables = this.getAllVariablesOfMetaType(typesToSearch, _Resources["default"].CONNECTION_TYPE);
    ...
    for (var x = 0; x < connectionVariables.length; x++) {
      ...
      if (!present) {
        ...
        if (pointerA.getPath() == instance.getId() || pointerB.getPath() == instance.getId()) {
          ...
          this.augmentPointer(pointerB, connectionInstance); // set A and B on connection
  ...
  /** Creates an instance */
  createExternalInstance: function createExternalInstance(path, projectId) {
    var options = {
      _metaType: _Resources["default"].INSTANCE_NODE,
      path: path,
      projectId: projectId
  ...
  createArrayElementInstance: function createArrayElementInstance(options) {
    if (options == null || options == undefined) {
      options = {
        _metaType: _Resources["default"].ARRAY_ELEMENT_INSTANCE_NODE
      };
  ...
  createArrayInstance: function createArrayInstance(options) {
    if (options == null || options == undefined) {
      options = {
        _metaType: _Resources["default"].ARRAY_INSTANCE_NODE
      };
  ...
  createVisualGroups: function createVisualGroups(nodes, parent) {
    var visualGroups = [];
    ...
    for (var i = 0; i < nodes.length; i++) {
      if (nodes[i].visualGroupElements != undefined) {
        ...
        if (tagRefObjs != undefined) {
          var tags = []; // populate tags from references
  ...
  createVisualGroupElements: function createVisualGroupElements(nodes, parent) {
   ...
   */
  cleanupInstanceTreeState: function cleanupInstanceTreeState() {
    // get state variables - clean out time series and watched status
    var stateVariableInstances = this.getAllInstancesOf(_Resources["default"].STATE_VARIABLE_TYPE_PATH);
    ...
    var parameterInstances = this.getAllInstancesOf(_Resources["default"].PARAMETER_TYPE_PATH);
   ...
   * @returns {Array}
   */
  getAllInstancesWithCapability: function getAllInstancesWithCapability(capabilityId, instances) {
    var matchingInstances = []; // traverse everything and populate matching instances
   ...
   */
  getAllInstancesOf: function getAllInstancesOf(typeOrVar, instances) {
    ...
    } else if (typeOrVar.getMetaType() == _Resources["default"].VARIABLE_NODE) {
   ...
   */
  getAllInstancesOfType: function getAllInstancesOfType(type, instances) {
    if (!(type instanceof Type)) {
      // raise hell
   ...
   */
  getAllInstancesOfVariable: function getAllInstancesOfVariable(variable, instances) {
    if (!(variable.getMetaType() == _Resources["default"].VARIABLE_NODE)) {
      // raise hell
   ...
   */
  getAllPotentialInstancesEndingWith: function getAllPotentialInstancesEndingWith(endingString) {
   ...
   */
  getAllPotentialInstancesStartingWith: function getAllPotentialInstancesStartingWith(startingString) {
    var matchingPotentialInstances = [];
   ...
   */
  getAllPotentialInstancesOfType: function getAllPotentialInstancesOfType(typePath, paths) {
    if (paths == undefined) {
      paths = this.allPaths;
    }
   ...
   */
  getAllPotentialInstancesOfMetaType: function getAllPotentialInstancesOfMetaType(metaType, paths, includeType) {
    if (paths == undefined) {
      paths = this.allPaths;
    }
   ...
   * @returns {Array} - Types
   */
  getAllTypesOfMetaType: function getAllTypesOfMetaType(metaType) {
    var types = []; // iterate all libraries
   ...
   * @returns {Array} - Types
   */
  getAllTypesOfType: function getAllTypesOfType(type) {
        ...
        if (libraryTypes[j] == type) {
          ...
          for (var w = 0; w < superTypes.length; w++) {
            if (superTypes[w] == type) {
              ...
              types.push(libraryTypes[j]); // sufficient condition met, break the loop

              break;
            }
          }
        } else {// TODO: no immediate matches - recurse on super type and see if any matches if any matches add this type
   ...
   * @returns {Array}
   */
  getAllVariablesOfType: function getAllVariablesOfType(typesToSearch, typeToMatch, recursive) {
    ...
    for (var i = 0; i < typesToSearch.length; i++) {
      if (typesToSearch[i].getMetaType() == _Resources["default"].COMPOSITE_TYPE_NODE) {
              ...
              if (varTypes[x] == typeToMatch) {
                ...
                for (var w = 0; w < superTypes.length; w++) {
                  if (superTypes[w] == typeToMatch) {
                    variables.push(nestedVariables[j]); // sufficient condition met, break the loop

                    break;
                  }
                }
              } else if (varTypes[x].getMetaType() == _Resources["default"].COMPOSITE_TYPE_NODE) {
   ...
   * @returns {Array}
   */
  getAllVariablesOfMetaType: function getAllVariablesOfMetaType(typesToSearch, metaType) {
    // check if array and if not "make it so"
   ...
   * @returns {Array}
   */
  getTopLevelVariablesById: function getTopLevelVariablesById(variableIds) {
    var variables = [];
   ...
   * @param resultType
   */
  getMatchingQueries: function getMatchingQueries(type, resultType) {
    var topLevelQueries = window.Model.getQueries();
    var matchingQueries = []; // iterate top level queries
  ...
  getHTMLVariable: function getHTMLVariable(typesToSearch, metaType, identifier) {
   ...
   * @param instances
   */
  getInstanceCount: function getInstanceCount(instances) {
    var count = 0;
   ...
   * @param instance
   */
  deleteInstance: function deleteInstance(instance, deleteCallback) {
    var instancePath = instance.getPath();

    var removeMatchingInstanceFromArray = function removeMatchingInstanceFromArray(instanceArray, instance) {
      var index = null;
    ...
    if (parent == undefined) {
       ...
       */
      removeMatchingInstanceFromArray(window.Instances, instance); // remove reference

      delete window[instance.getId()];
    } else {
      // remove from array of children
      removeMatchingInstanceFromArray(parent.getChildren(), instance); // remove reference
   ...
   * @param type
   */
  unresolveType: function unresolveType(type) {
    var libs = this.geppettoModel.getLibraries();
    var typePath = type.getPath(); // swap the type with type.overrideType if any is found
    ...
    if (type.overrideType != undefined) {
      ...
      var variablesToUpdate = type.getVariableReferences(); // swap type reference in ALL variables that point to it
      ...
      for (var m = 0; m < typeToLibraryMap[typePath].getTypes().length; m++) {
        if (type.getPath() == typeToLibraryMap[typePath].getTypes()[m].getPath()) {
          // swap type in raw model
          typeToLibraryMap[typePath].getWrappedObj().types[m] = type.overrideType.getWrappedObj(); // swap in object model (this line is probably redundant as the parent hasn't changed)
      ...
      this.populateTypeReferences(type.overrideType); // add potential instance paths

      this.addPotentialInstancePaths(variablesToUpdate); // update capabilities for variables and instances if any
   ...
   */
  resolve: function resolve(refStr) {
    ...
    if (!reference) {
      console.error("Error resolving reference ".concat(refStr));
    }

    return reference;
  },
  getVariables: function getVariables(rawGeppettoModel) {
    if (!rawGeppettoModel.worlds || !rawGeppettoModel.worlds.length) {
      return rawGeppettoModel.variables;
    }

    var world = rawGeppettoModel.worlds[0]; // TODO handle multiple worlds

    return world.variables;
  },
  fillWorldsFromRawModel: function fillWorldsFromRawModel(geppettoModel, jsonModel) {
    var _this3 = this;

    geppettoModel.worlds = jsonModel.worlds.map(function (world) {
      return _this3.createWorld(world);
    });
  },
  _getStaticInstancePaths: function _getStaticInstancePaths(geppettoModel) {
    if (geppettoModel.getCurrentWorld === undefined) {
      if (!geppettoModel.worlds || !geppettoModel.worlds.length) {
        return [];
      }

      var rawModel = geppettoModel;
      geppettoModel = new GeppettoModel({
        wrappedObj: rawModel
      });
