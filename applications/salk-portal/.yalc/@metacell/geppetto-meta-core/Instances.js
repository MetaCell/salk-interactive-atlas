...
exports.augmentInstancesArray = augmentInstancesArray;
 ...
 */
function augmentInstancesArray(instances) {
  ...
  instances.addInstances = function (instancePaths) {
    ...
    _ModelFactory["default"].addInstances(instancePaths, window.Instances, window.Model);
  };
  ...
  instances.getInstance = function (instancePath, create, override) {
    ...
    for (var j = 0; j < instancePath.length; j++) {
      if (instancePath[j].indexOf('[*]') > -1) {
        ...
        var arraySize = arrayInstance.getSize(); // remove original * entry

        instancePath.splice(j, 1); // add exploded elements
    ...
    for (var i = 0; i < instancePath.length; i++) {
      try {
        ...
        if (potentialVar != undefined) {
          if (override) {
            _ModelFactory["default"].deleteInstance(instances[i]);
...
var _default = {
  augmentInstancesArray: augmentInstancesArray
};
