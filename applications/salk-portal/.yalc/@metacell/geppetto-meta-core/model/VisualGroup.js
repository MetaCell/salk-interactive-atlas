...
VisualGroup.prototype.show = function (mode, instances) {
  ...
  if (instances == undefined) {
    var instances = _ModelFactory["default"].getAllInstancesOf(this.getParent());
  }

  if (mode) {
    message = _Resources["default"].SHOWING_VISUAL_GROUPS + this.id;
  } else {
    message = _Resources["default"].HIDING_VISUAL_GROUPS + this.id;
  }

  if (elements.length > 0) {
    this.showAllVisualGroupElements(elements, mode, instances);
  } else {
    message = _Resources["default"].NO_VISUAL_GROUP_ELEMENTS;
  }
...
VisualGroup.prototype.getMinDensity = function () {
  var allElements = [];
  var elements = this.getVisualGroupElements(); // calculate mean;
...
//# sourceMappingURL=VisualGroup.js.map